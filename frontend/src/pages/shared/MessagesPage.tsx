    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          {unreadCount > 0 && <p className="text-sm text-primary-600">{unreadCount} {t('messages.unread')}</p>}
        </div>
        <button
      ...
      <div className="flex gap-2">
        <button onClick={() => { setFolder('inbox'); setSelectedMsg(null); }}
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${folder === 'inbox' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-100'}`}>
          {t('messages.inbox')}
        </button>
        <button onClick={() => { setFolder('sent'); setSelectedMsg(null); }}
          className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors ${folder === 'sent' ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-100'}`}>
          {t('messages.sent')}
        </button>
      </div>

      {showCompose && (
        <div className="animate-slide-down rounded-xl border border-primary-100 bg-white p-6 shadow-md">
          <h2 className="mb-4 text-lg font-semibold text-gray-900">{t('messages.compose')}</h2>
          <form onSubmit={handleSend} className="space-y-4">
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-500">{t('messages.to')}</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-500">{t('messages.subject')}</label>
              <input type="text" value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })}
                className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm focus:border-primary-400 focus:ring-2 focus:ring-primary-100 focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-500">{t('messages.message')}</label>
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={() => setShowCompose(false)} className="rounded-lg border border-gray-200 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50">
                {t('common.cancel')}
              </button>
            </div>
          </form>
      ...
      {selectedMsg && (
        <div className="animate-slide-down rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100">
            <svg className="h-8 w-8 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
          </div>
          <p className="text-sm font-medium text-gray-900">{t('messages.noMessages')}</p>
        </div>
      ) : (
          ...
          <ul className="divide-y divide-gray-50">
            {messages.map((msg) => (
              <li key={msg.id} onClick={() => openMessage(msg)}
                className={`flex cursor-pointer items-center gap-3 px-5 py-3 transition-colors hover:bg-gray-50/60 ${!msg.is_read && msg.recipient === user?.id ? 'bg-primary-50/30' : ''}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className={`truncate text-sm ${!msg.is_read && msg.recipient === user?.id ? 'font-semibold text-gray-900' : 'font-medium text-gray-700'}`}>
                  ...
                  <p className="truncate text-xs text-gray-400">{msg.subject}</p>
                </div>
