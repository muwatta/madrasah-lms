...
export default function AdminEngagementPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">{t('engagement.title')}</h1>
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium text-gray-400">{t('engagement.weeklyActive')}</p>
            <p className="text-2xl font-bold text-gray-900">{data.weekly_active_students}</p>
          </div>
        </div>
      </div>

      <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('engagement.dailyAttempts')}</h2>
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('engagement.teacherStats')}</h2>
        {data.teacher_stats.length === 0 ? (
          <p className="text-sm text-gray-400">{language === 'ar' ? 'لا يوجد معلمون بعد' : 'No teachers yet'}</p>
        ) : (
          <div className="space-y-3">
            {data.teacher_stats.sort((a, b) => b.average_score - a.average_score).map((t, i) => (
              <div key={t.teacher_id} className="flex items-center gap-4 rounded-lg border border-gray-50 bg-gray-50/50 p-4">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-sm font-bold text-blue-700">
                  #{i + 1}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-gray-900">{t.name}</p>
                  <p className="text-xs text-gray-400">{t.student_count} {language === 'ar' ? 'طالب' : 'students'} • {t.total_attempts} {language === 'ar' ? 'محاولة' : 'attempts'}</p>
                </div>
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('engagement.subjectTrends')}</h2>
        {data.subject_trends.length === 0 ? (
          <p className="text-sm text-gray-400">{language === 'ar' ? 'لا توجد مواد بعد' : 'No subjects yet'}</p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {data.subject_trends.map((s) => (
              <div key={s.subject_id} className="rounded-lg border border-gray-50 bg-gray-50/50 p-4">
                <p className="text-sm font-semibold text-gray-900">{language === 'ar' ? s.name_ar : s.name_en}</p>
