  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between">
        <div className="mb-2">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{language === 'ar' ? 'عبء عمل المعلمين' : 'Teacher Workload'}</h1>
        </div>
      ...
      <p className="text-sm text-gray-500 dark:text-[var(--color-text-muted)] mb-6">{language === 'ar' ? 'عرض ومراقبة عبء عمل جميع المعلمين' : 'View and monitor all teachers\' workload'}</p>
      ...
      {sorted.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 dark:border-[var(--color-border)] bg-white dark:bg-[var(--color-bg-secondary)] py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-900 dark:text-[var(--color-text-primary)]">{language === 'ar' ? 'لا يوجد معلمين' : 'No teachers found'}</p>
          <p className="mt-1 text-xs text-gray-400 dark:text-[var(--color-text-muted)]">{language === 'ar' ? 'سيظهر عبء العمل هنا بعد إضافة المعلمين' : 'Workload will appear here after teachers are added'}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {sorted.map((tw, idx) => {
                ...
                <div className="p-5">
                  <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{tw.teacher_name}</h3>
                    ...
                    <span className="text-lg font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{total}</span>
                  </div>

                  <div className="mb-4">
                    <div className="flex items-center justify-between text-xs text-gray-400 dark:text-[var(--color-text-muted)]">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-gray-50 dark:bg-[var(--color-bg-primary)] p-3 text-center">
                      <p className="text-xs text-gray-400 dark:text-[var(--color-text-muted)]">{language === 'ar' ? 'خطط الدروس' : 'Lesson Plans'}</p>
                      <p className="mt-0.5 text-lg font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{tw.lesson_plans_count}</p>
                    </div>
                    <div className="rounded-lg bg-gray-50 dark:bg-[var(--color-bg-primary)] p-3 text-center">
                      <p className="text-xs text-gray-400 dark:text-[var(--color-text-muted)]">{language === 'ar' ? 'الواجبات' : 'Homework'}</p>
                      <p className="mt-0.5 text-lg font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{tw.homework_count}</p>
                    </div>
                    <div className="rounded-lg bg-gray-50 dark:bg-[var(--color-bg-primary)] p-3 text-center">
                      <p className="text-xs text-gray-400 dark:text-[var(--color-text-muted)]">{language === 'ar' ? 'تقييمات معلقة' : 'Ungraded'}</p>
                      <p className="mt-0.5 text-lg font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{tw.ungraded_submissions_count}</p>
                    </div>
                    <div className="rounded-lg bg-gray-50 dark:bg-[var(--color-bg-primary)] p-3 text-center">
                      <p className="text-xs text-gray-400 dark:text-[var(--color-text-muted)]">{language === 'ar' ? 'دروس قادمة' : 'Upcoming'}</p>
                      <p className="mt-0.5 text-lg font-bold text-gray-900 dark:text-[var(--color-text-primary)]">{tw.upcoming_lessons}</p>
                    </div>
                  </div>
                </div>
              </div>
