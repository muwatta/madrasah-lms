  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{t('student.quizzes')}</h1>
        <select
          value={filterSubject}
          onChange={(e) => setFilterSubject(e.target.value ? Number(e.target.value) : '')}
          className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-emerald-500 outline-none"
        >
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{t('guides.quizList')}</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500 dark:text-gray-400">{t('student.noQuizzesAvailable')}</div>
      ) : groupedQuizzes ? (
        <div className="space-y-8">
          {groupedQuizzes.map((group) => (
            <div key={group.subject.id}>
              <div className="flex items-center gap-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100">{language === 'ar' ? group.subject.name_ar : group.subject.name_en}</h2>
                <span className="text-sm text-gray-400 dark:text-gray-500">({group.quizzes.length})</span>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {group.quizzes.map((quiz) => {
                  ...
                  const avgScore = completedAttempts.length > 0
                    ...
                    : null;
                  return (
                    <div key={quiz.id} className="bg-white dark:bg-gray-800 rounded-xl shadow border border-emerald-100 dark:border-emerald-900/30 p-6 flex flex-col">
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-2">{quiz.title}</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-2">{quiz.description}</p>
                        <div className="flex flex-wrap gap-2 text-xs">
                          <span className="px-2 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">{quiz.subject_name}</span>
                          ...
                          <span className="px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{quiz.question_count} {t('student.questions')}</span>
                          {quiz.time_limit_minutes && (
                            <span className="px-2 py-1 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">{quiz.time_limit_minutes} {t('student.minutes')}</span>
                          )}
                        </div>
                        {completedAttempts.length > 0 && (
                          <div className="mt-3 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                            <span>{completedAttempts.length} {t('quizManagement.attempts')}</span>
                            <span className={`font-semibold ${avgScore !== null && avgScore >= quiz.passing_score ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
                              {avgScore}%
                            </span>
                            <span className="text-gray-400 dark:text-gray-500">{t('fields.passingScore')}: {quiz.passing_score}%</span>
                          </div>
                        )}
                      </div>
                      <Link
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {quizzes.map((quiz) => {
            ...
            return (
              <div key={quiz.id} className="bg-white dark:bg-gray-800 rounded-xl shadow border border-emerald-100 dark:border-emerald-900/30 p-6 flex flex-col">
                <div className="flex-1">
                  <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-2">{quiz.title}</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-2">{quiz.description}</p>
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="px-2 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">{quiz.subject_name}</span>
                    <span className="px-2 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">
