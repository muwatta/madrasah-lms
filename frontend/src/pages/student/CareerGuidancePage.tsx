  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Career Guidance</h1>
          <p className="text-gray-500 mt-1">Discover career paths and opportunities tailored to your strengths</p>
        </div>
        <button
      ...
      {!current ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-800 mb-2">No Recommendations Yet</h2>
        ...
        <div className="space-y-8">
          <div>
            <h2 className="text-lg font-semibold text-gray-800 mb-4">Recommended Careers</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {current.recommendations.map((career, i) => (
                <div
                  key={i}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 opacity-0 animate-slide-up"
                  style={{ animationDelay: `${i * 80}ms` }}
                >
                  <h3 className="font-semibold text-gray-800 text-lg mb-2">{career.career}</h3>
                  <p className="text-sm text-gray-600 mb-4">{career.description}</p>
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs font-medium text-gray-400 uppercase mb-1">Required Skills</p>
                      <div className="flex flex-wrap gap-1.5">
                        {career.required_skills.map((skill, j) => (
                          <span key={j} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700">
                            {skill}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <div>
                        <p className="text-xs font-medium text-gray-400">Avg. Salary</p>
                        <p className="font-medium text-gray-700">{career.avg_salary}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-medium text-gray-400">Growth Outlook</p>
                        <p className="font-medium text-emerald-600">{career.growth_outlook}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              <h2 className="text-lg font-semibold text-gray-800 mb-4">Recommended Universities</h2>
              <div className="space-y-3">
                {current.recommended_universities.map((uni, i) => (
                  <div
                    key={i}
                    className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-start gap-4 opacity-0 animate-slide-up"
