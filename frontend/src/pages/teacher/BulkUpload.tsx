    ...
    <div className="rounded-xl border border-dashed border-primary-200 bg-primary-50/30 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-800">{t('bulkUpload.title')}</h3>
        </div>
      ...
      <p className="mb-4 text-xs text-gray-500">{t('bulkUpload.templateHint')}</p>
      ...
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition-colors ${
          dragging ? 'border-primary-400 bg-primary-50' : 'border-gray-200 bg-white hover:border-primary-300 hover:bg-primary-50/30'
        }`}
      >
        ...
        {uploading ? (
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600">{t('bulkUpload.uploading')}</p>
          </div>
        ) : (
          <div className="text-center">
      ...
      {fileName && !uploading && (
        <p className="mt-2 text-xs text-gray-500">{language === 'ar' ? 'الملف:' : 'File:'} {fileName}</p>
      )}
      ...
      {result && (
        <div className={`mt-4 rounded-lg p-3 text-sm ${result.errors.length === 0 ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
          ...
          {result.errors.length > 0 ? (
            ...
            <p className="text-xs text-green-600">{t('bulkUpload.noErrors')}</p>
          ) : null}
        </div>
      )}
    </div>
