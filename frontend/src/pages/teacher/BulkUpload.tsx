...
export default function BulkUpload({ onComplete }: { onComplete: () => void }) {
  ...
  const downloadTemplate = () => {
    const headers = ['question_text', 'question_type', 'options', 'correct_answer', 'explanation', 'difficulty', 'topic_id'];
    const sample = [
      ['ما هي أول سورة في القرآن؟', 'mcq', 'الفاتحة|البقرة|آل عمران|النساء', 'الفاتحة', 'سورة الفاتحة هي أول سورة', 'easy', ''],
      ['كم عدد سور القرآن؟', 'fill_blank', '', '114', 'يحتوي القرآن على 114 سورة', 'medium', ''],
      ['ما معنى التوحيد؟', 'short_answer', '', 'التوحيد هو الإيمان بوحدانية الله', 'التوحيد مفهوم إسلامي أساسي', 'medium', ''],
    ];
  ...
  const handleFile = async (file: File) => {
    ...
    setResult(null);
    try {
      const res = await questionAPI.bulkUpload(file);
      setResult(res.data);
      onComplete();
    } catch (err: any) {
    ...
    <div className="rounded-xl border border-dashed border-primary-200 dark:border-primary-800 bg-primary-50/30 dark:bg-primary-900/20 p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">{t('bulkUpload.title')}</h3>
        </div>
      ...
      <p className="mb-4 text-xs text-gray-500 dark:text-gray-400">{t('bulkUpload.templateHint')}</p>
      ...
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition-colors ${
          dragging ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/30' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-primary-300 hover:bg-primary-50/30 dark:hover:bg-primary-900/20'
        }`}
      >
        ...
        {uploading ? (
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">{t('bulkUpload.uploading')}</p>
          </div>
        ) : (
          <div className="text-center">
        ...
        <div className={`mt-4 rounded-lg p-3 text-sm ${result.errors.length === 0 ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400'}`}>
          {result.created > 0 && (
            <p className="font-medium">
              {result.created} {t('bulkUpload.created')}
            </p>
          )}
          {result.errors.length > 0 ? (
            ...
            <p className="text-xs text-green-600 dark:text-green-400">{t('bulkUpload.noErrors')}</p>
          ) : null}
        </div>
      )}
    </div>
