  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Link to="/teacher/quizzes" className="text-sm text-primary-600 hover:text-primary-800 mb-1 inline-block">
            {language === 'ar' ? 'العودة للاختبارات' : '← Back to Quizzes'}
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">{data.quiz.title}</h1>
          <p className="text-sm text-gray-500">{t('quizAnalytics.title')}</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-gray-400">{t('quizAnalytics.totalAttempts')}</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{data.total_attempts}</p>
        </div>
        <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-gray-400">{t('quizAnalytics.averageScore')}</p>
          <p className={`mt-1 text-2xl font-bold ${getAccuracyColor(data.average_score).split(' ')[0]}`}>{data.average_score}%</p>
        </div>
        <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-gray-400">{t('quizAnalytics.passRate')}</p>
          <p className="mt-1 text-2xl font-bold text-blue-600">{data.pass_rate}%</p>
        </div>
        <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
          <p className="text-xs font-medium text-gray-400">{language === 'ar' ? 'نقطة النجاح' : 'Passing Score'}</p>
          <p className="mt-1 text-2xl font-bold text-gray-900">{data.quiz.passing_score}%</p>
        </div>
      </div>

      {data.score_distribution.length > 0 && (
        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('quizAnalytics.scoreDistribution')}</h2>
          <div className="flex items-end gap-1.5" style={{ height: 120 }}>
            {data.score_distribution.map((d) => (
              <div key={d.range} className="flex flex-1 flex-col items-center gap-1">
                ...
                <div
                  className={`w-full rounded-t-md transition-all ${d.count > 0 ? 'bg-primary-400' : 'bg-gray-100'}`}
                  style={{ height: `${d.count > 0 ? (d.count / maxDistCount) * 100 : 4}%` }}
                />
                <span className="text-[9px] text-gray-400">{d.range}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {Object.keys(data.difficulty_breakdown).length > 0 && (
        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('quizAnalytics.difficultyBreakdown')}</h2>
          <div className="grid grid-cols-3 gap-4">
            {Object.entries(data.difficulty_breakdown).map(([diff, info]) => (
              <div key={diff} className="text-center">
                ...
                <p className="mt-2 text-lg font-bold text-gray-900">{info.total}</p>
                <p className="text-xs text-gray-400">{language === 'ar' ? 'سؤال' : 'questions'}</p>
                <div className="mt-2 h-2 w-full rounded-full bg-gray-100">
                  <div className={`h-2 rounded-full ${getBarColor(info.avg_accuracy)}`} style={{ width: `${info.avg_accuracy}%` }} />
                </div>
                <p className="mt-1 text-xs text-gray-500">{info.avg_accuracy}% {language === 'ar' ? 'متوسط الدقة' : 'avg accuracy'}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.question_analytics.length > 0 && (
        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-gray-700">{t('quizAnalytics.questionPerformance')}</h2>
          <div className="space-y-3">
            {data.question_analytics.map((q, i) => (
              <div key={q.question_id} className="rounded-lg border border-gray-50 bg-gray-50/50 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-600">{q.question_type}</span>
                    </div>
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-gray-400">{q.correct_count}/{q.total_answered} {language === 'ar' ? 'صحيح' : 'correct'}</span>
                </div>
                {q.top_wrong_answers.length > 0 && (
                  <div className="mt-2">
                    <p className="text-[10px] font-medium text-gray-400 mb-1">{t('quizAnalytics.commonWrongAnswers')}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {q.top_wrong_answers.map((wa, wi) => (
                        <span key={wi} className="rounded bg-red-50 px-2 py-0.5 text-[10px] text-red-600">
                          "{wa.answer}" ({wa.count})
                        </span>
                      ))}
                    </div>
      ...
      {data.total_attempts === 0 && (
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-900">{t('quizAnalytics.noData')}</p>
        </div>
      )}
    </div>
  );
