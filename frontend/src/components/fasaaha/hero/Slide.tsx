...
import { BookOpen, Mic, GraduationCap, Target, Trophy, Award, BarChart3, MessageSquare, PenTool } from 'lucide-react';
...
const cardIcons: Record<string, React.ReactNode> = {
  ...
  Pronunciation: <Target className="w-4 h-4" />,
  Grammar: <PenTool className="w-4 h-4" />,
  Fluency: <MessageSquare className="w-4 h-4" />,
  Vocabulary: <BookOpen className="w-4 h-4" />,
  Conversation: <MessageSquare className="w-4 h-4" />,
  'Students Active': <GraduationCap className="w-4 h-4" />,
  'Missions Done': <Trophy className="w-4 h-4" />,
  'Avg. Score': <Award className="w-4 h-4" />,
    ...
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'Score', value: '87%', color: 'from-emerald-400/20' },
            { label: 'Completion', value: '94%', color: 'from-blue-400/20' },
            { label: 'Accuracy', value: '82%', color: 'from-amber-400/20' },
            { label: 'Streak', value: '12d', color: 'from-violet-400/20' },
          ].map((metric, i) => (
            <motion.div
              key={metric.label}
